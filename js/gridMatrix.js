/** Game-configured ones-digit matrix renderer. */
...
export class GridMatrix {
  ...
  setDraws(draws, game = 'cash5', options = {}) {
    this.draws = draws || [];
    this.gameConfig = getGameConfig(game);
  ...
  render() {
    ...
    if (!this.draws.length) {
      this.container.innerHTML = `<div class="matrix-empty"><strong>No draw data available</strong><span>Import a ${escapeHTML(this.gameConfig.displayName)} CSV or restore sample data from the Data menu.</span></div>`;
      return;
    }
    ...
    const header = `<thead><tr><th>Draw date</th>${Array.from({ length: this.gameConfig.ballCount }, (_, index) => (
      `<th colspan="${columnSpan}">Ball ${index + 1}</th>${index < this.gameConfig.ballCount - 1 ? '<th class="ball-sep"></th>' : ''}`
    )).join('')}</tr></thead>`;
    ...
    const body = this.draws.map(draw => {
      ...
      const cells = draw.numbers.map((number, column) => {
        ...
        const ones = showOnes
          ...
          : '';
        return `${tens}${ones}${column < this.gameConfig.ballCount - 1 ? '<td class="ball-sep"></td>' : ''}`;
      }).join('');
  ...
  updateHighlights() {
    this.container.querySelectorAll('.square-cell').forEach(cell => {
      ...
      for (let position = 1; position <= this.gameConfig.ballCount; position += 1) cell.classList.remove(`position-${position}`);
